...
export default function HaveList() {
  ...
  const toggleReserved = (id: string) => {
    setHaveItems(items =>
      items.map(item =>
        item.id === id ? { ...item, isReserved: !item.isReserved, reservedFor: item.isReserved ? undefined : 'D2Gambit' } : item
      )
    );
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="flex items-center space-x-2 bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors" onClick={handleAddHaveItemClick}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-500">
                        <div className="space-y-1">
                          <div>Found by: <span className="font-medium text-gray-700">{item.foundBy}</span></div>
                          <div>Location: <span className="font-medium text-gray-700">{item.location}</span></div>
                        </div>
                        <div className="space-y-1">
                          <div>Date found: <span className="font-medium text-gray-700">{item.dateFound}</span></div>
                          {item.isReserved && item.reservedFor && (
                            <div>Reserved for: <span className="font-medium text-red-600">{item.reservedFor}</span></div>
                          )}
                        </div>
                      </div>
                    </div>
        ...
        {/* Modal for Add Item */}
        {isModalOpen && <HaveItemForm isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} haveItems={haveItems} setHaveItems={setHaveItems} addItem={addItem} />}
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="text-2xl font-bold text-green-600">{haveItems.filter(i => !i.isReserved).length}</div>
            <div className="text-sm text-gray-600">Available</div>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="text-2xl font-bold text-red-600">{haveItems.filter(i => i.isReserved).length}</div>
            <div className="text-sm text-gray-600">Reserved</div>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="text-2xl font-bold text-orange-600">{haveItems.filter(i => i.quality === 'Unique').length}</div>
