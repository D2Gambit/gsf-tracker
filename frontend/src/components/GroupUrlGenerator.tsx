...
const GroupUrlGenerator = () => {
  const [groupUrl, setGroupUrl] = useState('https://www.gsftracker.com/group/CariGSF-JAN2026');
  ...
  return (
    <section className="bg-zinc-300 rounded-lg border border-gray-200 p-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label htmlFor="groupUrl" className="block text-sm font-medium text-gray-700 mb-2">
            Shareable Group URL
          </label>
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <h3 className="font-medium text-red-800 mb-2">Share this URL with your group members</h3>
          <p className="text-sm text-red-700">
            Anyone with this URL can join your GSF group and contribute to the shared item lists.
          </p>
        </div>

         {/* Group Members Section */}
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Group Members ({players.length})</h3>
          <div className="space-y-3">
            {players.map((player) => (
              <div key={player.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <div className="font-medium text-gray-900">{player.accountName}</div>
                  <div className="text-sm text-gray-600">Character: {player.characterName}</div>
                </div>
                <button
