...
export default function AddItemForm({ isModalOpen, setIsModalOpen, needItems, setNeedItems, addItem }: { isModalOpen: boolean, setIsModalOpen: React.Dispatch<React.SetStateAction<boolean>>, needItems: any[], setNeedItems: React.Dispatch<React.SetStateAction<any[]>>, addItem: (id: string) => void }) {
    ...
    const handleAddItemConfirmClick = async () => {
        try {
            ...
            setNeedItems([...needItems, { ...form, dateAdded: new Date(Date.now()).toLocaleDateString('en-CA'), isActive: true }])
       ...
       (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                <div className="bg-zinc-800 w-full max-w-lg rounded-lg p-6">
                  ...
                  {/* Priority*/}
                  <label className="block mb-2 text-sm font-medium text-zinc-100">Priority</label>
                  <select
                    className="w-full mb-3 p-2 rounded bg-zinc-700 text-zinc-100"
                    value={form.priority}
