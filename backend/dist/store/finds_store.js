...
export const getHotFinds = async (gsfGroupId, limit = 3) => {
    return db
        .select({
        id: finds.id,
        ...
        reactionCount: sql `count(${findReactions.id})`,
    })
        .from(finds)
...
export const getLatestFinds = async (gsfGroupId, limit, cursor) => {
    let query = db
        .select()
        .from(finds)
        .where(eq(finds.gsfGroupId, gsfGroupId))
    ...
    if (cursor) {
        query = db
            .select()
            .from(finds)
            .where(and(eq(finds.gsfGroupId, gsfGroupId), or(lt(finds.createdAt, cursor.createdAt), and(eq(finds.createdAt, cursor.createdAt), lt(finds.id, parseInt(cursor.id))))))
            .orderBy(desc(finds.createdAt), desc(finds.id))
            .limit(limit + 1);
    }
...
export const getGsfReactions = async (gsfGroupId) => {
    return db
        .select()
        .from(findReactions)
